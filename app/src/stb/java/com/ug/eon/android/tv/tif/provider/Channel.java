...
/**
 * Created by milan.adamovic on 2/12/18.
 */

public final class Channel {
    public static final String[] PROJECTION = getProjection();
    public static final String[] SIMPLE_PROJECTION = getSimpleProjection();
    ...
    private Channel() {
        ...
        mInternalFlag1 = INVALID_INTEGER_VALUE;
    ...
    public InternalProviderData getInternalProviderData() {
        if (mInternalProviderData != null) {
            ...
            } catch (InternalProviderData.ParseException ignore) {
                return null;
            }
        }
        return null;
    ...
    public int getmInternalFlag1() {
        return mInternalFlag1;
    }
    ...
    public ContentValues toContentValues() {
        ...
        if (mBrowsable == false) {
            values.put(TvContract.Channels.COLUMN_BROWSABLE, 0);
        } else {
            values.put(TvContract.Channels.COLUMN_BROWSABLE, 1);
        }

        if (mLocked == false) {
            values.put(TvContract.Channels.COLUMN_LOCKED, 0);
        } else {
            values.put(TvContract.Channels.COLUMN_LOCKED, 1);
        }
        ...
        values.put(TvContract.Channels.COLUMN_INTERNAL_PROVIDER_FLAG1, mInternalFlag1);
    ...
    public static Channel fromCursor(Cursor cursor) {
        ...
        if(!cursor.isNull(++index)) {
            builder.setBrowsable(cursor.getInt(index) == 1);
        }

        if (!cursor.isNull(++index)) {
            builder.setLocked(cursor.getInt(index) == 1) ;
        }
        ...
        if (!cursor.isNull(++index)) {
            try {
                builder.setInternalProviderData(new InternalProviderData(cursor.getBlob(index)));
            } catch (InternalProviderData.ParseException e) {
                e.printStackTrace();
            }
        }
        if (!cursor.isNull(++index)) {
            builder.setInternalFlag1(cursor.getInt(index));
        }
        return builder.build();
    }
    ...
    private static String[] getSimpleProjection() {
        return new String[] {
                TvContract.Channels._ID,
                TvContract.Channels.COLUMN_INPUT_ID,
                TvContract.Channels.COLUMN_ORIGINAL_NETWORK_ID,
    ...
    public static final class Builder {
        ...
        public Builder setInternalFlag1(int channelId) {
            mChannel.mInternalFlag1 = channelId;
            return this;
        }
